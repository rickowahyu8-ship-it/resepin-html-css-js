...
/**
 * Daftar token bahan unik dari seluruh database (dipecah dari `Ingredients
 * Cleaned` yang sudah dipastikan konsisten dipisah koma), diurutkan dari
 * yang paling sering muncul. Dipakai sebagai sumber data untuk filter
 * bahan (dropdown/checkbox) di UI -- bukan bahan karangan, murni dari data.
 */
export function getIngredientTokens() {
  ...
  const counts = new Map();
  for (const r of getAllRecipes()) {
    const parts = r.ingredientsCleaned.split(',').map((s) => s.trim()).filter(Boolean);
